...
const ItemCheck = ({ uncheckItem, id }: { uncheckItem: TCheckItem, id: number; }) => (
  <div className="w-5 h-5 rounded-full bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center  hover:scale-120 transition-transform"
  ...
  return (
    <div className="p-4 flex gap-4 group hover:bg-gray-750 relative" key={item.id}>
      ...
      <img
        src="/icon-cross.svg"
        alt="Delete"
        className="w-4 h-4 absolute right-4 top-1/2 -translate-y-1/2 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer pointer-events-auto hover:brightness-150"
